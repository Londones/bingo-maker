...
const Page = () => {
    ...
    useEffect(() => {
        if (session?.user?.id && isClient && authorToken) {
            const bingoIds = JSON.parse(localStorage.getItem("ownedBingos") || "[]");
            if (bingoIds.length > 0) {
                // Pass the bingo IDs as query parameters to the /me page
                const searchParams = new URLSearchParams();
                searchParams.append("bingoIds", JSON.stringify(bingoIds));
                searchParams.append("authorToken", authorToken);

                // Clear the ownedBingos from localStorage since they'll be migrated
                localStorage.removeItem("ownedBingos");

                router.push(`/me?${searchParams.toString()}`);
            } else {
                router.push("/me");
            }
        }
    }, [session?.user?.id, isClient, authorToken, router]);
