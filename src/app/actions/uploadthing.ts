...
export async function uploadPendingImages(state: Bingo): Promise<ImageUploadResponse> {
  ...
  if (cellImages.length > 0) {
    ...
    for (const image of cellImages) {
      try {
        ...
        throw new Error(`Failed to upload cell image: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  }
  ...
  if (backgroundImage) {
    try {
      ...
      throw new Error(`Failed to upload background image: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  console.log("Upload completed, returning URLs:", result);
  return result;
}
